...
sub lastMap{
  ...
  while(<$sout>){
    ...
    if((index $name1, "Adapter") != -1){
      
      if(!exists($queryScore_Both{$name2}) || ($queryScore_Both{$name2} < $score)){
        $queryScore_Both{$name2} = $score;
        ...
        my ($bcFixed, $bcActual) = split /_Adapter_/, $name1;
        my $bcOnlyId = $bcFixed . $bcActual;
        
        if (exists($targetTracker{$bcOnlyId})) {
      ...
      if(!exists($queryScore_OnlyBC{$name2}) || ($queryScore_OnlyBC{$name2} < $score)){
        $queryScore_OnlyBC{$name2} = $score;
        # default to keep start of sequence
        my $keepStart = 0;
        my $keepEnd = $start2;
        if(($start2 + ($alnSize2 / 2)) < ($seqSize2 / 2)){
        ...
        my ($bcFixed, $bcActual) = split /_/, $name1;
        my $bcWithAdapterId = $bcFixed . '_Adapter_' . $bcActual;
        
        if (exists($targetTracker{$bcWithAdapterId})) {
